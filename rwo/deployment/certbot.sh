...
set -x
certbot $@ $CERTBOT_ARGS -d $DOMAIN_NAME -d www.$DOMAIN_NAME --email hostmaster@$DOMAIN_NAME --manual --preferred-challenges=http --manual-auth-hook ./certbot-helper-auth.sh --manual-cleanup-hook ./certbot-helper-cleanup.sh --deploy-hook ./certbot-helper-deploy.sh --agree-tos -n --config-dir $CERTBOT_FOLDER/conf --work-dir $CERTBOT_FOLDER/work --logs-dir $CERTBOT_FOLDER/log
